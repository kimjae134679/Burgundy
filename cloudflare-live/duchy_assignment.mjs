...
export function canUseClassical(playerCount=2){
  // Physical-style classical setup needs two distinct options per player and no duplicate duchies.
...
export function assignDuchyIds({mode='beginner_same',chosenId=1,playerCount=2,rngInt=n=>Math.floor(Math.random()*n)}={}){
  ...
  if(mode==='classical'){
    ...
    const pool=[...verified];
    // The caller/UI will eventually expose the two choices. Until then pick one from each unique pair.
    const out=[];
    for(let i=0;i<n;i++){
